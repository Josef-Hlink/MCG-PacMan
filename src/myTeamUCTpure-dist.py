...
class UCTAgent(CaptureAgent):
    ...
    def chooseAction(self, gameState: GameState) -> str:
        ...
        heuristic = False
        if (rolloutVal == 0 or pos[0] in (1,2)) and not pos[0]>10:
            # use heuristic
            heuristic = True
            bAct = self.getBestActionHeuristic(gameState)
        else:
    ...
    def rollout(self, gameState: GameState) -> float:
        ...
        while not state.isOver() and depth < self.rolloutDepth:
            ...
            actions.remove('Stop')
            # current_score = self.evaluate(gameState)
            bestAction = random.choice(actions)
            # for action in actions:
            #     successor = state.generateSuccessor(self.index, action)
            #     foodList = self.getFood(successor).asList()
            #     pos2 = successor.getAgentPosition(self.index)
            #     minDistance = min([self.getMazeDistance(pos2, food) for food in foodList])
            #     new_score = self.evaluate(successor)
        
            #     if minDistance < bestDist or new_score>current_score:
            #         bestAction = action
            #         bestDist = minDistance
            # print(bestDist)
            state = state.generateSuccessor(self.index, bestAction)
            depth += 1
        return self.evaluate(state)
    
    def getBestActionHeuristic(self, state: GameState) -> str:
        bestDist = 9999
        actions = state.getLegalActions(self.index)
        actions.remove('Stop')
        bestAction = random.choice(actions)
    ...
    def log(self, action: str, heuristic: bool) -> None:
        ...
        print(f'Agent {pIdx} {pPos} -> {action} -> {nextPos}  |  score: {currentScore} |  heuristic: {heuristic}')
